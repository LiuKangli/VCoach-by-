   ...
   // Start the HTTP connection
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1 withObject:self];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1 withObject:self];
		}
	}	
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[ASIHTTPRequest performSelector:@selector(setMaxValue:newMax:) onTarget:&uploadProgressDelegate withObject:self amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[ASIHTTPRequest performSelector:@selector(setMaxValue:newMax:) onTarget:&downloadProgressDelegate withObject:self amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize] withObject:self];
...
- (void)updateUploadProgress
{
	...
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize] withObject:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength] withObject:self];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total withObject:(id)object
{
	#if TARGET_OS_IPHONE
		// Cocoa Touch: UIProgressView
		SEL selector = @selector(setProgress:newProgress:);
		float progressAmount = (float)((progress*1.0)/(total*1.0));
		
	#else
		// Cocoa: NSProgressIndicator
		double progressAmount = progressAmount = (progress*1.0)/(total*1.0);
		SEL selector = @selector(setDoubleValue:newProgress:);
	#endif
	...
	[progressLock lock];
	[ASIHTTPRequest performSelector:selector onTarget:indicator withObject:object amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
